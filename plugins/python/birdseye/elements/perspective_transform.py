...
class Perspective_Transform:
    ...
    def homography_matrix(self, image):
        ...
        data = sio.loadmat(
            os.path.join(self.current_directory, "data/worldcup2014.mat")
        )
        ...
        result, _ = flann.nn(
            ...
            test_features[self.query_index],
            1,
            algorithm="kdtree",
            trees=8,
            checks=64,
        ...
        Warp_img = cv2.warpPerspective(
            seg_map, np.linalg.inv(refined_h), (115, 74), borderMode=cv2.BORDER_CONSTANT
        )
    ...
    def generate_deep_feature(self, edge_map):
        ...
        data_loader = CameraDataset(
            pivot_image, pivot_image, batch_size, -1, data_transform, is_train=False
        )
