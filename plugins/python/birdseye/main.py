...
def main(opt):
    ...
    # Ensure the black image is loaded correctly
    if gt_img is None:
        raise FileNotFoundError(
            "Black image for the soccer field ('./inference/black.jpg') not found."
        )

    # Ensure video dimensions are valid
    if w <= 0 or h <= 0:
        raise ValueError(
            "Invalid video dimensions. Check if the video source is correct."
        )
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            if yoloOutput:
                ...
                for i, obj in enumerate(yoloOutput):
                    ...
                    if obj["label"] == "player":
                        ...
                        try:
                            color = detect_color(
                                main_frame[xyxy[1] : xyxy[3], xyxy[0] : xyxy[2]]
                            )
            ...
            frame[
                frame.shape[0] - bg_img.shape[0] :, frame.shape[1] - bg_img.shape[1] :
            ] = bg_img
