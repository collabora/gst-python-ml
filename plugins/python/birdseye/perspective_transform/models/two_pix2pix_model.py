...
class TwoPix2PixModel:
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.seg_netG = networks.define_G(
                opt.input_nc,
                opt.output_nc,
                opt.ngf,
            ...
            self.detec_netG = networks.define_G(
                opt.input_nc,
                opt.output_nc,
                opt.ngf,
                opt.which_model_netG,
        ...
        if opt.continue_train:
            ...
            if self.isTrain:
                self.load_network(
                    self.segmentation_GAN.netD, "D", opt.which_epoch, "seg"
                )
    ...
    def backward_D(self):
        if self.isJointTrain:
            ...
            # combined loss
            self.loss_D = (
                self.segmentation_GAN.loss_D_fake
                + self.segmentation_GAN.loss_D_real
                + self.detection_GAN.loss_D_fake
                + self.detection_GAN.loss_D_real
            ) * 0.5
    ...
    def backward_G(self):
        if self.isJointTrain:
            ...
            self.segmentation_GAN.loss_G_L1 = (
                seg_GAN.criterionL1(self.fake_B, self.real_B) * seg_GAN.opt.lambda_A
            )
            ...
            self.detection_GAN.loss_G_L1 = (
                detect_GAN.criterionL1(self.fake_D, self.real_D)
                * detect_GAN.opt.lambda_A
            )

            self.loss_G = (
                self.segmentation_GAN.loss_G_GAN
                + self.segmentation_GAN.loss_G_L1
