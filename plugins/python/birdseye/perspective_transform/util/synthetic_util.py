...
class SyntheticUtil:
    @staticmethod
    def camera_to_edge_image(
        camera_data, model_points, model_line_segment, im_h=720, im_w=1280, line_width=4
    ):
        """
         #720,1280
    ...
    def find_transform(im_src, im_dst):
        ...
        try:
            (_, warp) = cv.findTransformECC(
                im_src, im_dst, warp, cv.MOTION_HOMOGRAPHY, criteria, None, 1
            )
        except:
    ...
    def generate_ptz_cameras(
        ...
        for i in range(camera_num):
            base_rotation = (
                RotationUtil.rotate_y_axis(0)
                @ RotationUtil.rotate_x_axis(rolls[i])
    ...
    def sample_positive_pair(
        pp, cc, base_roll, pan, tilt, fl, pan_std, tilt_std, fl_std
    ):
        """
        ...
        base_rotation = (
            RotationUtil.rotate_y_axis(0)
            @ RotationUtil.rotate_y_axis(base_roll)
            @ RotationUtil.rotate_x_axis(-90)
        )
    ...
    def generate_database_images(
        pivot_cameras, positive_cameras, model_points, model_line_segment
    ):
        """
